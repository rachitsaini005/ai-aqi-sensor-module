...
3. **Host app** (`host/`) trains a `RandomForestRegressor` (100 trees) on [`data/AQI_Prediction_IoT_50.csv`](data/README.md), predicts the AQI for each incoming line, labels it (Good ≤ 50, Moderate ≤ 100, otherwise Unhealthy) and appends it to `aqi_history.csv`.
...
1. Set `ESP_PORT` in `aqi_host.py` to your serial port (for example `COM3` on Windows or `/dev/ttyUSB0` on Linux).
2. Close the Arduino Serial Monitor (it holds the port), then run:
...
`data/AQI_Prediction_IoT_50.csv` has 50 rows and the nine columns the host app expects (`Temp, Hum, PM25, PM10, CO, NO2, SO2, O3, AQI`). The pollutant values and AQI are real daily CPCB records for Amritsar (2017–2020). **Temp and Hum are synthetic** (approximate Amritsar monthly climate), because the source has no weather data. See [`data/README.md`](data/README.md) for the sources, units, caveats and how to rebuild or replace it.
...
data/                         Training dataset, its provenance and the script that builds it
images/                       Figures
```
